...
test('hides the password quality by default', () => {
    const tree = renderer.create(<PasswordInput
        ...
        name="test"
        onChange={() => {}}
        value="123412412431234" />).toJSON()
    expect(tree).toMatchSnapshot()
})

test('shows the password quality when enabled and a password is entered', () => {
    const tree = renderer.create(<PasswordInput
        htmlId="test"
        name="test"
        onChange={() => {}}
...
test('hides the password quality when enabled but no password is entered', () => {
    const tree = renderer.create(<PasswordInput
        htmlId="test"
        name="test"
        onChange={() => {}}
